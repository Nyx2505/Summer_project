var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var operations_exports = {};
__export(operations_exports, {
  $delete: () => $delete,
  _$deleteDeserialize: () => _$deleteDeserialize,
  _$deleteSend: () => _$deleteSend,
  _createOrUpdateAtManagementGroupDeserialize: () => _createOrUpdateAtManagementGroupDeserialize,
  _createOrUpdateAtManagementGroupSend: () => _createOrUpdateAtManagementGroupSend,
  _createOrUpdateDeserialize: () => _createOrUpdateDeserialize,
  _createOrUpdateSend: () => _createOrUpdateSend,
  _deleteAtManagementGroupDeserialize: () => _deleteAtManagementGroupDeserialize,
  _deleteAtManagementGroupSend: () => _deleteAtManagementGroupSend,
  _getAtManagementGroupDeserialize: () => _getAtManagementGroupDeserialize,
  _getAtManagementGroupSend: () => _getAtManagementGroupSend,
  _getBuiltInDeserialize: () => _getBuiltInDeserialize,
  _getBuiltInSend: () => _getBuiltInSend,
  _getDeserialize: () => _getDeserialize,
  _getSend: () => _getSend,
  _listBuiltInDeserialize: () => _listBuiltInDeserialize,
  _listBuiltInSend: () => _listBuiltInSend,
  _listByManagementGroupDeserialize: () => _listByManagementGroupDeserialize,
  _listByManagementGroupSend: () => _listByManagementGroupSend,
  _listDeserialize: () => _listDeserialize,
  _listSend: () => _listSend,
  createOrUpdate: () => createOrUpdate,
  createOrUpdateAtManagementGroup: () => createOrUpdateAtManagementGroup,
  deleteAtManagementGroup: () => deleteAtManagementGroup,
  get: () => get,
  getAtManagementGroup: () => getAtManagementGroup,
  getBuiltIn: () => getBuiltIn,
  list: () => list,
  listBuiltIn: () => listBuiltIn,
  listByManagementGroup: () => listByManagementGroup
});
module.exports = __toCommonJS(operations_exports);
var import_models = require("../../models/models.js");
var import_pagingHelpers = require("../../static-helpers/pagingHelpers.js");
var import_urlTemplate = require("../../static-helpers/urlTemplate.js");
var import_core_client = require("@azure-rest/core-client");
function _listByManagementGroupSend(context, managementGroupId, options = { requestOptions: {} }) {
  const path = (0, import_urlTemplate.expandUrlTemplate)(
    "/providers/Microsoft.Management/managementGroups/{managementGroupId}/providers/Microsoft.Authorization/policySetDefinitions{?api%2Dversion,%24filter,%24expand,%24top}",
    {
      managementGroupId,
      "api%2Dversion": context.apiVersion ?? "2025-03-01",
      "%24filter": options?.filter,
      "%24expand": options?.expand,
      "%24top": options?.top
    },
    {
      allowReserved: options?.requestOptions?.skipUrlEncoding
    }
  );
  return context.path(path).get({
    ...(0, import_core_client.operationOptionsToRequestParameters)(options),
    headers: { accept: "application/json", ...options.requestOptions?.headers }
  });
}
async function _listByManagementGroupDeserialize(result) {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    const error = (0, import_core_client.createRestError)(result);
    error.details = (0, import_models.errorResponseDeserializer)(result.body);
    throw error;
  }
  return (0, import_models._policySetDefinitionListResultDeserializer)(result.body);
}
function listByManagementGroup(context, managementGroupId, options = { requestOptions: {} }) {
  return (0, import_pagingHelpers.buildPagedAsyncIterator)(
    context,
    () => _listByManagementGroupSend(context, managementGroupId, options),
    _listByManagementGroupDeserialize,
    ["200"],
    { itemName: "value", nextLinkName: "nextLink", apiVersion: context.apiVersion ?? "2025-03-01" }
  );
}
function _deleteAtManagementGroupSend(context, managementGroupId, policySetDefinitionName, options = { requestOptions: {} }) {
  const path = (0, import_urlTemplate.expandUrlTemplate)(
    "/providers/Microsoft.Management/managementGroups/{managementGroupId}/providers/Microsoft.Authorization/policySetDefinitions/{policySetDefinitionName}{?api%2Dversion}",
    {
      managementGroupId,
      policySetDefinitionName,
      "api%2Dversion": context.apiVersion ?? "2025-03-01"
    },
    {
      allowReserved: options?.requestOptions?.skipUrlEncoding
    }
  );
  return context.path(path).delete({ ...(0, import_core_client.operationOptionsToRequestParameters)(options) });
}
async function _deleteAtManagementGroupDeserialize(result) {
  const expectedStatuses = ["200", "204"];
  if (!expectedStatuses.includes(result.status)) {
    const error = (0, import_core_client.createRestError)(result);
    error.details = (0, import_models.errorResponseDeserializer)(result.body);
    throw error;
  }
  return;
}
async function deleteAtManagementGroup(context, managementGroupId, policySetDefinitionName, options = { requestOptions: {} }) {
  const result = await _deleteAtManagementGroupSend(
    context,
    managementGroupId,
    policySetDefinitionName,
    options
  );
  return _deleteAtManagementGroupDeserialize(result);
}
function _createOrUpdateAtManagementGroupSend(context, managementGroupId, policySetDefinitionName, parameters, options = {
  requestOptions: {}
}) {
  const path = (0, import_urlTemplate.expandUrlTemplate)(
    "/providers/Microsoft.Management/managementGroups/{managementGroupId}/providers/Microsoft.Authorization/policySetDefinitions/{policySetDefinitionName}{?api%2Dversion}",
    {
      managementGroupId,
      policySetDefinitionName,
      "api%2Dversion": context.apiVersion ?? "2025-03-01"
    },
    {
      allowReserved: options?.requestOptions?.skipUrlEncoding
    }
  );
  return context.path(path).put({
    ...(0, import_core_client.operationOptionsToRequestParameters)(options),
    contentType: "application/json",
    headers: { accept: "application/json", ...options.requestOptions?.headers },
    body: (0, import_models.policySetDefinitionSerializer)(parameters)
  });
}
async function _createOrUpdateAtManagementGroupDeserialize(result) {
  const expectedStatuses = ["200", "201"];
  if (!expectedStatuses.includes(result.status)) {
    const error = (0, import_core_client.createRestError)(result);
    error.details = (0, import_models.errorResponseDeserializer)(result.body);
    throw error;
  }
  return (0, import_models.policySetDefinitionDeserializer)(result.body);
}
async function createOrUpdateAtManagementGroup(context, managementGroupId, policySetDefinitionName, parameters, options = {
  requestOptions: {}
}) {
  const result = await _createOrUpdateAtManagementGroupSend(
    context,
    managementGroupId,
    policySetDefinitionName,
    parameters,
    options
  );
  return _createOrUpdateAtManagementGroupDeserialize(result);
}
function _getAtManagementGroupSend(context, managementGroupId, policySetDefinitionName, options = { requestOptions: {} }) {
  const path = (0, import_urlTemplate.expandUrlTemplate)(
    "/providers/Microsoft.Management/managementGroups/{managementGroupId}/providers/Microsoft.Authorization/policySetDefinitions/{policySetDefinitionName}{?api%2Dversion,%24expand}",
    {
      managementGroupId,
      policySetDefinitionName,
      "api%2Dversion": context.apiVersion ?? "2025-03-01",
      "%24expand": options?.expand
    },
    {
      allowReserved: options?.requestOptions?.skipUrlEncoding
    }
  );
  return context.path(path).get({
    ...(0, import_core_client.operationOptionsToRequestParameters)(options),
    headers: { accept: "application/json", ...options.requestOptions?.headers }
  });
}
async function _getAtManagementGroupDeserialize(result) {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    const error = (0, import_core_client.createRestError)(result);
    error.details = (0, import_models.errorResponseDeserializer)(result.body);
    throw error;
  }
  return (0, import_models.policySetDefinitionDeserializer)(result.body);
}
async function getAtManagementGroup(context, managementGroupId, policySetDefinitionName, options = { requestOptions: {} }) {
  const result = await _getAtManagementGroupSend(
    context,
    managementGroupId,
    policySetDefinitionName,
    options
  );
  return _getAtManagementGroupDeserialize(result);
}
function _listBuiltInSend(context, options = { requestOptions: {} }) {
  const path = (0, import_urlTemplate.expandUrlTemplate)(
    "/providers/Microsoft.Authorization/policySetDefinitions{?api%2Dversion,%24filter,%24expand,%24top}",
    {
      "api%2Dversion": context.apiVersion ?? "2025-03-01",
      "%24filter": options?.filter,
      "%24expand": options?.expand,
      "%24top": options?.top
    },
    {
      allowReserved: options?.requestOptions?.skipUrlEncoding
    }
  );
  return context.path(path).get({
    ...(0, import_core_client.operationOptionsToRequestParameters)(options),
    headers: { accept: "application/json", ...options.requestOptions?.headers }
  });
}
async function _listBuiltInDeserialize(result) {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    const error = (0, import_core_client.createRestError)(result);
    error.details = (0, import_models.errorResponseDeserializer)(result.body);
    throw error;
  }
  return (0, import_models._policySetDefinitionListResultDeserializer)(result.body);
}
function listBuiltIn(context, options = { requestOptions: {} }) {
  return (0, import_pagingHelpers.buildPagedAsyncIterator)(
    context,
    () => _listBuiltInSend(context, options),
    _listBuiltInDeserialize,
    ["200"],
    { itemName: "value", nextLinkName: "nextLink", apiVersion: context.apiVersion ?? "2025-03-01" }
  );
}
function _getBuiltInSend(context, policySetDefinitionName, options = { requestOptions: {} }) {
  const path = (0, import_urlTemplate.expandUrlTemplate)(
    "/providers/Microsoft.Authorization/policySetDefinitions/{policySetDefinitionName}{?api%2Dversion,%24expand}",
    {
      policySetDefinitionName,
      "api%2Dversion": context.apiVersion ?? "2025-03-01",
      "%24expand": options?.expand
    },
    {
      allowReserved: options?.requestOptions?.skipUrlEncoding
    }
  );
  return context.path(path).get({
    ...(0, import_core_client.operationOptionsToRequestParameters)(options),
    headers: { accept: "application/json", ...options.requestOptions?.headers }
  });
}
async function _getBuiltInDeserialize(result) {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    const error = (0, import_core_client.createRestError)(result);
    error.details = (0, import_models.errorResponseDeserializer)(result.body);
    throw error;
  }
  return (0, import_models.policySetDefinitionDeserializer)(result.body);
}
async function getBuiltIn(context, policySetDefinitionName, options = { requestOptions: {} }) {
  const result = await _getBuiltInSend(context, policySetDefinitionName, options);
  return _getBuiltInDeserialize(result);
}
function _listSend(context, options = { requestOptions: {} }) {
  const path = (0, import_urlTemplate.expandUrlTemplate)(
    "/subscriptions/{subscriptionId}/providers/Microsoft.Authorization/policySetDefinitions{?api%2Dversion,%24filter,%24expand,%24top}",
    {
      subscriptionId: context.subscriptionId,
      "api%2Dversion": context.apiVersion ?? "2025-03-01",
      "%24filter": options?.filter,
      "%24expand": options?.expand,
      "%24top": options?.top
    },
    {
      allowReserved: options?.requestOptions?.skipUrlEncoding
    }
  );
  return context.path(path).get({
    ...(0, import_core_client.operationOptionsToRequestParameters)(options),
    headers: { accept: "application/json", ...options.requestOptions?.headers }
  });
}
async function _listDeserialize(result) {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    const error = (0, import_core_client.createRestError)(result);
    error.details = (0, import_models.errorResponseDeserializer)(result.body);
    throw error;
  }
  return (0, import_models._policySetDefinitionListResultDeserializer)(result.body);
}
function list(context, options = { requestOptions: {} }) {
  return (0, import_pagingHelpers.buildPagedAsyncIterator)(
    context,
    () => _listSend(context, options),
    _listDeserialize,
    ["200"],
    { itemName: "value", nextLinkName: "nextLink", apiVersion: context.apiVersion ?? "2025-03-01" }
  );
}
function _$deleteSend(context, policySetDefinitionName, options = { requestOptions: {} }) {
  const path = (0, import_urlTemplate.expandUrlTemplate)(
    "/subscriptions/{subscriptionId}/providers/Microsoft.Authorization/policySetDefinitions/{policySetDefinitionName}{?api%2Dversion}",
    {
      subscriptionId: context.subscriptionId,
      policySetDefinitionName,
      "api%2Dversion": context.apiVersion ?? "2025-03-01"
    },
    {
      allowReserved: options?.requestOptions?.skipUrlEncoding
    }
  );
  return context.path(path).delete({ ...(0, import_core_client.operationOptionsToRequestParameters)(options) });
}
async function _$deleteDeserialize(result) {
  const expectedStatuses = ["200", "204"];
  if (!expectedStatuses.includes(result.status)) {
    const error = (0, import_core_client.createRestError)(result);
    error.details = (0, import_models.errorResponseDeserializer)(result.body);
    throw error;
  }
  return;
}
async function $delete(context, policySetDefinitionName, options = { requestOptions: {} }) {
  const result = await _$deleteSend(context, policySetDefinitionName, options);
  return _$deleteDeserialize(result);
}
function _createOrUpdateSend(context, policySetDefinitionName, parameters, options = { requestOptions: {} }) {
  const path = (0, import_urlTemplate.expandUrlTemplate)(
    "/subscriptions/{subscriptionId}/providers/Microsoft.Authorization/policySetDefinitions/{policySetDefinitionName}{?api%2Dversion}",
    {
      subscriptionId: context.subscriptionId,
      policySetDefinitionName,
      "api%2Dversion": context.apiVersion ?? "2025-03-01"
    },
    {
      allowReserved: options?.requestOptions?.skipUrlEncoding
    }
  );
  return context.path(path).put({
    ...(0, import_core_client.operationOptionsToRequestParameters)(options),
    contentType: "application/json",
    headers: { accept: "application/json", ...options.requestOptions?.headers },
    body: (0, import_models.policySetDefinitionSerializer)(parameters)
  });
}
async function _createOrUpdateDeserialize(result) {
  const expectedStatuses = ["200", "201"];
  if (!expectedStatuses.includes(result.status)) {
    const error = (0, import_core_client.createRestError)(result);
    error.details = (0, import_models.errorResponseDeserializer)(result.body);
    throw error;
  }
  return (0, import_models.policySetDefinitionDeserializer)(result.body);
}
async function createOrUpdate(context, policySetDefinitionName, parameters, options = { requestOptions: {} }) {
  const result = await _createOrUpdateSend(context, policySetDefinitionName, parameters, options);
  return _createOrUpdateDeserialize(result);
}
function _getSend(context, policySetDefinitionName, options = { requestOptions: {} }) {
  const path = (0, import_urlTemplate.expandUrlTemplate)(
    "/subscriptions/{subscriptionId}/providers/Microsoft.Authorization/policySetDefinitions/{policySetDefinitionName}{?api%2Dversion,%24expand}",
    {
      subscriptionId: context.subscriptionId,
      policySetDefinitionName,
      "api%2Dversion": context.apiVersion ?? "2025-03-01",
      "%24expand": options?.expand
    },
    {
      allowReserved: options?.requestOptions?.skipUrlEncoding
    }
  );
  return context.path(path).get({
    ...(0, import_core_client.operationOptionsToRequestParameters)(options),
    headers: { accept: "application/json", ...options.requestOptions?.headers }
  });
}
async function _getDeserialize(result) {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    const error = (0, import_core_client.createRestError)(result);
    error.details = (0, import_models.errorResponseDeserializer)(result.body);
    throw error;
  }
  return (0, import_models.policySetDefinitionDeserializer)(result.body);
}
async function get(context, policySetDefinitionName, options = { requestOptions: {} }) {
  const result = await _getSend(context, policySetDefinitionName, options);
  return _getDeserialize(result);
}
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  $delete,
  _$deleteDeserialize,
  _$deleteSend,
  _createOrUpdateAtManagementGroupDeserialize,
  _createOrUpdateAtManagementGroupSend,
  _createOrUpdateDeserialize,
  _createOrUpdateSend,
  _deleteAtManagementGroupDeserialize,
  _deleteAtManagementGroupSend,
  _getAtManagementGroupDeserialize,
  _getAtManagementGroupSend,
  _getBuiltInDeserialize,
  _getBuiltInSend,
  _getDeserialize,
  _getSend,
  _listBuiltInDeserialize,
  _listBuiltInSend,
  _listByManagementGroupDeserialize,
  _listByManagementGroupSend,
  _listDeserialize,
  _listSend,
  createOrUpdate,
  createOrUpdateAtManagementGroup,
  deleteAtManagementGroup,
  get,
  getAtManagementGroup,
  getBuiltIn,
  list,
  listBuiltIn,
  listByManagementGroup
});
